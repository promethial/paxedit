...
(comment (message -!-"hello world")) ⟹ (message -!-"hello world")
      #+END_SRC
   10. ~paxedit-cleanup~ - Indent the buffer according to the rules of the LISP mode.
...
(+ 1-!-2)
       #+END_SRC
** Debugging
   1. ~paxedit-macro-expand-replace~ - Expand the current expression if it is macro.
      #+BEGIN_SRC emacs-lisp
